 ...
 */
    ...
    public:
        ...
        public:
            ...
            typename TreeNode::QVector *                qVec;       // qVector for the next choice point (set during messageToReward)
            ...
            message_type actToMessage(action_type act) {
                ...
                if(isOnTree()) treeNode = treeNode->getChildOrCreate(message);
                return message;
            }
...
//        };
        ...
        explicit IncompleteInformationMCTS(std::function<const BODY &()> priorAgentSampler);
...
//            { mind.train(observation, act, 0.0, observation, true) }; // TODO: send reward only? More flexible? or add messages? or be more flexible with observation?
    ...
    void IncompleteInformationMCTS<BODY>::buildTree(int nSamples, double discount) {
        typedef Agent<SelfPlayBody,SelfPlayMind> agent_type;
        Agent<SelfPlayBody,SelfPlayMind> a(SelfPlayBody(), SelfPlayMind(discount));
        societies::RandomEncounterSociety soc({
            Agent(SelfPlayBody(), SelfPlayMind(discount)),
            Agent(SelfPlayBody(), SelfPlayMind(discount))
        });
