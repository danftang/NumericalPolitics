...
namespace abm {
    template<int NSTATES, int NACTIONS, class QVALUE = ExponentiallyWeightedQValue<100>>
    class QTable {
    public:
...
//        std::array<std::array<uint, NACTIONS>, NSTATES>      nSamples;
        ...
        std::array<abm::QVector<NACTIONS, QVALUE>, NSTATES>  Qtable;
...
//        uint trainingStepsSinceLastPolicyChange = 0;
         ...
         */
        QTable(double discount, double sampleDecay = 0.999) : discount(discount), sampleDecay(sampleDecay) { }
...
//        }
        ...
        QVector<NACTIONS> QVector(size_t body) { return Qtable[body]; }
    };
}
